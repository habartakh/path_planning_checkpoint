...
bool DijkstraGlobalPlanner::dijkstraShortestPath(
  ...
  // As long as there are still unvisited nodes
  while (!open_list.empty()) {

    // Sort nodes ordered by ascending 'g_cost' value
    std::sort(open_list.begin(), open_list.end(),
              [](std::pair<int, double> &a, std::pair<int, double> &b) {
                return a.second < b.second;
              });

    // get the first element of open_list, ie the closest one
    ...
    // Verify if the goal node wa reached
    if (current_node == goal_cell_index) {
      RCLCPP_INFO(node_->get_logger(), "Dijkstra: Goal Reached");
      path_found = true;
      break;
    }
    ...
    for (auto neighbour : neighbours) {
      ...
      if (closed_list.find(neighbour_index) != closed_list.end()) {
        // Neighbour already visited, go to the following neigbour
        continue;
      }

      // Compute cost from the start node to the neighbour passing by the
      // current_node
      double g_cost = g_costs[current_node] + step_cost;

      // Check if the neighbour has been visited or not yet
      auto neigbour_iter =
          std::find_if(open_list.begin(), open_list.end(),
                       [neighbour_index](const std::pair<int, double> &a) {
                         return (a.first == neighbour_index);
                       });

      // If the neighbor is already inside open_list
      if (neigbour_iter != open_list.end()) {
      ...
      else { // the neighbor has not been visted yet
